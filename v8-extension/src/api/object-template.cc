    ...
    void ObjectTemplate::constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
        ...
        if (info.Length() >= 1) {
            if (!info[0]->IsNullOrUndefined()) {
                ...
                auto holder = info[0].As<v8::Object>()->FindInstanceInPrototypeChain(FunctionTemplate::get_template(isolate));
                if (holder.IsEmpty()) {
                    JS_THROW_ERROR(NOTHING, context, TypeError, "arguments[0]: not a 'FunctionTemplate' object");
                }
